---
title: 'Introduction to Program R I: The structure of R'
...
You can also add a comment after (but not before) a code chunk that you wish to evaluate. This is often very useful in proving a description of a given step. Let's add a comment after the evaluated code and run this section:
...
To convert a character value to a factor, you can use the "factor" function. Run the following and note how the output differs from that of the character:
...
* **Matrix**: A two-dimensional collection of values, arranged as rows and columns. Component vectors MUST be of the same class. If this is not the case, the class of the component values will be modified.
...
* **Data frame**: A two-dimensional collection of values, arranged as rows and columns. Component vectors may be of different classes. 
...
* **List**: An n-dimensional collection of objects, arranged as list items denoted by "[[i]]". May contain individual values, vectors, data frames, and matrices of different classes and dimensions.
...
Values within an R object such as a vector, matrix, data frame, or list can be referred to by their location within the object.

#### Using vector location
...
3. Use the combine function to extract the third through fifth values in the rabbits vector.
...
4. Use the nrow and ncol functions to extract the last row and column of dataframe1.
...
> **Exercise Six:**
>
> 1. Create a vector that consists of the number values five through ten.
2. Create a vector that consists of letter values a through f.
3. Create a list object that contains the two vector elements.
4. Create a data frame that contains the two vectors.
5. Name the list elements "numbers" and "letters".
6. Name the columns of the data frame "numbers" and "letters".
7. Extract the fourth value of the numbers column in the data frame using the row number and column name.
8. Extract the first and fourth values of the letters list element.
9. In the numbers list element, change the number 7 to 17.
...
We will now read in data into R. First, we need to load a library (RCurl) that allows us to read the data directly from our course github page. Run the function "require" R to load the library:

```{r, eval=FALSE}
require(RCurl)
```

Next, we supply the location of the data using the function "getURL":

```{r, eval=FALSE}
fileLocation <- getURL('https://raw.githubusercontent.com/SCBI-MigBirds/MigBirds/master/data/exampleBirdData.csv')
```

Finally, we read the data into R using the "read.csv" function. We use the "text" argument to let R know the location of the file:

```{r, eval=FALSE}
data <- read.csv(text = fileLocation)
```
...
>**Exercise Seven:** 
>
>1. Determine the class of the R object "data" and its components.
2. Determine the number of columns of the data object.
3. Determine the number of rows of the data object.
4. Extract the vector of names associated  with the data object.
...
The function "head" shows the first six rows of data. If we use the "n" argument, we can specify the number of rows we would like to show in the console:
...
```{r, eval = F}
length(unique(data$diet))
```

We may further be interested in the number of observations across factor levels. To do so, we can use the "table" function to tabulate the factor counts for a given vector within the dataset:

```{r, eval=FALSE}
table(data$diet)
```

Providing basic summary statistics is often a crucial part of data exploration. Summary statistics of interest often include:

* Minimum: `min(data$count)`
* Maximum: `max(data$count)`
* Mean: `mean(data$count)`
* Median: `median(data$count)`
* Standard deviation: `sd(data$count)`

Finally, we can view summary statistics for the dataset across fields using the "summary" function:
...
> **Exercise Eight:** For the example dataset, address the following questions:
>
>1. What are the factor levels for the column "foraging"?
2. How many unique species does the dataset contain?
3. What is the mean value of foraging counts (Hint: this is a nested function!)?
