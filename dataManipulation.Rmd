...
> **Exercise 1**:
> 1. Run the following script to read wideData.csv into R:
>
>```{r, eval=FALSE}
> library(RCurl)
> fileURL <- getURL('https://raw.githubusercontent.com/SCBI-MigBirds/MigBirds/master/data/wideCounts.csv')
> wideCounts <- read.csv(text = fileURL)
>```
> Convert the wide count data frame to a long format tidy dataset.
>
> Run the following script to read wideData.csv into R:
>
>```{r, eval=FALSE}
> fileURL <- getURL('https://raw.githubusercontent.com/SCBI-MigBirds/MigBirds/master/data/longCounts.csv')
> longCounts <- read.csv(text = fileURL)
>```
> Convert the long count data frame to a wide format tidy dataset.
>

----

For the sake of brevity, we'll leave tidying data to just these two functions. There are a few other functions in tidyr (I strongly suggest reading the tidyr paper reading some of the tidyr resources), but you'll have to explore those on your own to give us some time to explore the wonderful world of dplyr!
