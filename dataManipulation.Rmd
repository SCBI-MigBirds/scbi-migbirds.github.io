...
In the above, each bird is certainly an observation, but is each distance class (which represents 10 to 30 m from the point count location) truly a separate variable? Nope. As such, we are be compelled tidy our data by collapsing the distances into an individual column. In the script below we use the `tidyr` function `gather` as:
