...
> **Exercise 5**    
> The `paste` function in base R is used to concatenate character values. It can be used as paste(column1, column2). Use mutate and the paste function to create a new column names "forDiet" in the birdCountsPrank data frame that concatenates values in the foraging and diet vectors.
>
...
The biggest asset of `dplyr` is in providing a flexible and memory efficient tool for summarizing data, the function `summarize`. This allows you to calculate summary statistics across a data frame. For example, to calculate the sum of counts, you would use:
