...
In the above, each bird is certainly an observation, but is each distance class (which represents 10 to 30 m from the point count location) truly a separate variable? Nope. As such, we are be compelled to collapse our distances to an individual column (if we believe in tidy data). In the script below we use the `tidyr` function `gather` by providing:

1. The data frame ("wide frame")
2. The name of the new "key" field for which we are gathering the data ("distance")
...
tidyCounts <- gather(wideFrame, distance, obs, d10:d30)
tidyCounts
```
...
>
> 1. Convert birdCounts to a wide format dataset.
